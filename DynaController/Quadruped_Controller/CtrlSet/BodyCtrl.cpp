...
#include <Atlas_Controller/TaskSet/SelectedJPosTask.hpp>
    ...
    Kd_(atlas::num_act_joint)
{
    ...
    //body_pos_task_ = new LinkHeightTask(robot, atlas_link::pelvis);
    body_pos_task_ = new LinkPosTask(robot, atlas_link::pelvis);
    body_ori_task_ = new LinkOriTask(robot, atlas_link::pelvis);
    torso_ori_task_ = new LinkOriTask(robot, atlas_link::torso);
...
void BodyCtrl::OneStep(void* _cmd){
    ...
    dynacore::Vector gamma = dynacore::Vector::Zero(atlas::num_act_joint);
    _contact_setup();
    _task_setup();
...
void BodyCtrl::_task_setup(){
    ...
    dynacore::Vector jvel_des(atlas::num_act_joint); jvel_des.setZero();
    dynacore::Vector jacc_des(atlas::num_act_joint); jacc_des.setZero();
...
void BodyCtrl::FirstVisit(){
    jpos_ini_ = sp_->Q_.segment(atlas::num_virtual, atlas::num_act_joint);
