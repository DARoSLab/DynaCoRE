...
SingleFullContact::SingleFullContact(const RobotSystem* robot, int pt):
    WBDC_ContactSpec(6),
...
SingleFullContact::~SingleFullContact(){  }

bool SingleFullContact::_UpdateJc(){
    robot_sys_->getFullJacobian(contact_pt_, Jc_);

    //dynacore::pretty_print(Jc_, std::cout, "Jc");
    return true;
}
bool SingleFullContact::_UpdateJcDotQdot(){
    robot_sys_->getFullJDotQdot(contact_pt_, JcDotQdot_);

    return true;
}

bool SingleFullContact::_UpdateUf(){
    double mu (0.5);
    double X(0.1);
    double Y(0.1);
  ...
  //dynacore::pretty_print(Rot_foot, std::cout, "rot foot");
  //Uf_ = U * Rot_foot;
  //TEST
  Uf_ = U;
...
bool SingleFullContact::_UpdateInequalityVector(){
    ieq_vec_ = dynacore::Vector::Zero(dim_U_);
    ieq_vec_[dim_U_-1] = -max_Fz_;
    return true;
}

void SingleFullContact::_setU(double x, double y, double mu, dynacore::Matrix & U){
