...
#include <Cheetah3_Controller/CtrlSet/DoubleContactTransCtrl.hpp>
#include <Cheetah3_Controller/CtrlSet/BodyCtrl.hpp>
#include <Cheetah3_Controller/CtrlSet/SingleContactTransCtrl.hpp>
#include <Cheetah3_Controller/CtrlSet/BodyFootPlanningCtrl.hpp>
    ...
    num_step_(0)
{
    ...
    sp_->stance_foot_ = cheetah3_link::leftFoot;
    ...
    right_swing_start_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, cheetah3_link::rightFoot, false);
    right_swing_end_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, cheetah3_link::rightFoot, true);
    // Left
    left_swing_start_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, cheetah3_link::leftFoot, false);
    left_swing_end_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, cheetah3_link::leftFoot, true);
    // Swing Controller Selection
    right_swing_ctrl_ = 
        new BodyFootPlanningCtrl(robot_sys_, cheetah3_link::rightFoot, reversal_planner_);
    left_swing_ctrl_ = 
        new BodyFootPlanningCtrl(robot_sys_, cheetah3_link::leftFoot, reversal_planner_);
...
void WalkingTest::TestInitialization(){
    ...
    reversal_planner_->PlannerInitialization(
            Cheetah3ConfigPath"PLANNER_velocity_reversal");
    // Yaml file name
...
int WalkingTest::_NextPhase(const int & phase){
    ...
    if(phase == WkPhase::double_contact_1) {
        ...
        robot_sys_->getPos(cheetah3_link::leftFoot, next_local_frame_location);
    ...
    if(phase == WkPhase::double_contact_2){
        ++num_step_;
        sp_->stance_foot_ = cheetah3_link::rightFoot;

        // Global Frame Update
        dynacore::Vect3 next_local_frame_location;
        robot_sys_->getPos(cheetah3_link::rightFoot, next_local_frame_location);
        sp_->global_pos_local_ += next_local_frame_location;
...
void WalkingTest::_SettingParameter(){
    // Setting Parameters
    ParamHandler handler(Cheetah3ConfigPath"TEST_walking.yaml");
