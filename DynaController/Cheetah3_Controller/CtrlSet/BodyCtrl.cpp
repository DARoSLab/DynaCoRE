    ...
    Kd_(cheetah3::num_act_joint)
{
    ...
    body_pos_task_ = new LinkPosTask(robot, cheetah3_link::body);
    body_ori_task_ = new LinkOriTask(robot, cheetah3_link::body);
    ...
    int idx_offset(0);
    wblc_data_->W_rf_[idx_offset + fr_contact_->getFzIndex()] = 0.01;
    idx_offset += fr_contact_->getDim();
    wblc_data_->W_rf_[ idx_offset + fl_contact_->getFzIndex()] = 0.01;
    idx_offset += fl_contact_->getDim();
    wblc_data_->W_rf_[ idx_offset + hr_contact_->getFzIndex()] = 0.01;
    idx_offset += hr_contact_->getDim();
    wblc_data_->W_rf_[ idx_offset + hl_contact_->getFzIndex()] = 0.01;
...
void BodyCtrl::OneStep(void* _cmd){
    ...
    dynacore::Vector gamma = dynacore::Vector::Zero(cheetah3::num_act_joint);
    _contact_setup();
    _task_setup();
...
void BodyCtrl::_task_setup(){
    ...
    dynacore::Vector jvel_des(cheetah3::num_act_joint); jvel_des.setZero();
    dynacore::Vector jacc_des(cheetah3::num_act_joint); jacc_des.setZero();
...
void BodyCtrl::FirstVisit(){
    jpos_ini_ = sp_->Q_.segment(cheetah3::num_virtual, cheetah3::num_act_joint);
