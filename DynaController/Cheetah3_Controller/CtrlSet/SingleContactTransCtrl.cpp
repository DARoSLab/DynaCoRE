...
void SingleContactTransCtrl::_task_setup(){
    ...
    if(b_increase_){
        if(moving_foot_ == cheetah3_link::rightFoot){
            int swing_jidx = cheetah3_joint::r_leg_hpz - cheetah3::num_virtual;
            double h(state_machine_time_/end_time_);

            for(int i(0); i < cheetah3::num_leg_joint; ++i){
                des_jpos_[i + swing_jidx] *= h; 
                des_jpos_[i + swing_jidx] += 
                    (1. - h)*sp_->des_jpos_prev_[swing_jidx + i];
             }
        }
        else if(moving_foot_ == cheetah3_link::leftFoot){
            int swing_jidx = cheetah3_joint::l_leg_hpz - cheetah3::num_virtual;
            double h(state_machine_time_/end_time_);
...
void SingleContactTransCtrl::_contact_setup(){
    ...
    int jidx_offset(0);
    if(moving_foot_ == cheetah3_link::leftFoot) {
    ...
    else if(moving_foot_ == cheetah3_link::rightFoot) {
