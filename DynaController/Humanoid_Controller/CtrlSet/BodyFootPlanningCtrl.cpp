    ...
    Kd_(humanoid::num_act_joint)
{
    ...
    body_pos_task_ = new LinkPosTask(robot_sys_, humanoid_link::torso);
    body_ori_task_ = new LinkOriTask(robot_sys_, humanoid_link::torso);
    ...
    int rf_idx_offset(0);
    if(swing_foot == humanoid_link::leftFoot) {
    ...
    else if(swing_foot == humanoid_link::rightFoot) {
...
void BodyFootPlanningCtrl::_task_setup(){
    ...
    dynacore::Vector zero(humanoid::num_act_joint); zero.setZero();
...
void BodyFootPlanningCtrl::_Replanning(dynacore::Vect3 & target_loc){
    ...
    if(swing_foot_ == humanoid_link::leftFoot)
...
bool BodyFootPlanningCtrl::EndOfPhase(){
    ...
    if(b_contact_switch_check_){
        ...
        if(swing_foot_ == humanoid_link::leftFoot && sp_->b_lfoot_contact_){
            contact_happen = true;
        }
        if(swing_foot_ == humanoid_link::rightFoot && sp_->b_rfoot_contact_){
            contact_happen = true;
        }
...
void BodyFootPlanningCtrl::CtrlInitialization(
    ...
    ini_base_height_ = sp_->Q_[humanoid_joint::virtual_Z];
