...
#include <Humanoid_Controller/TaskSet/LinkPosTask.hpp>
#include <Humanoid_Controller/TaskSet/LinkHeightTask.hpp>
#include <Humanoid_Controller/TaskSet/LinkOriTask.hpp>
#include <Humanoid_Controller/TaskSet/JPosTask.hpp>

#include <WBLC/KinWBC.hpp>
#include <WBLC/WBLC.hpp>
#include <Humanoid/Humanoid_Model.hpp>
#include <Humanoid_Controller/Humanoid_DynaCtrl_Definition.h>
#include <ParamHandler/ParamHandler.hpp>

#include <Humanoid_Controller/ContactSet/SingleContact.hpp>
#include <Humanoid_Controller/ContactSet/PointContact.hpp>
#include <Humanoid_Controller/ContactSet/PitchContact.hpp>
...
SingleContactTransCtrl::SingleContactTransCtrl(RobotSystem* robot, 
    ...
    end_time_(100.),
    ctrl_start_time_(0.),
    des_jpos_(humanoid::num_act_joint),
    des_jvel_(humanoid::num_act_joint),
    des_jacc_(humanoid::num_act_joint),
    Kp_(humanoid::num_act_joint),
    Kd_(humanoid::num_act_joint)
{

    total_joint_task_ = new JPosTask();
    body_pos_task_ = new LinkPosTask(robot, humanoid_link::torso);
    //body_pos_task_ = new LinkHeightTask(robot, humanoid_link::torso);
    body_ori_task_ = new LinkOriTask(robot, humanoid_link::torso);

    //rfoot_contact_ = new PointContact(robot_sys_, humanoid_link::rightFoot);
    //lfoot_contact_ = new PointContact(robot_sys_, humanoid_link::leftFoot);

    rfoot_contact_ = new PitchContact(robot_sys_, humanoid_link::rightFoot);
    lfoot_contact_ = new PitchContact(robot_sys_, humanoid_link::leftFoot);

     dim_contact_ = rfoot_contact_->getDim() + lfoot_contact_->getDim();

    std::vector<bool> act_list;
    act_list.resize(humanoid::num_qdot, true);
    for(int i(0); i<humanoid::num_virtual; ++i) act_list[i] = false;

    kin_wbc_ = new KinWBC(act_list);
    wblc_ = new WBLC(act_list);
    wblc_data_ = new WBLC_ExtraData();
    wblc_data_->W_qddot_ = dynacore::Vector::Constant(humanoid::num_qdot, 100.0);
    ...
    wblc_data_->tau_min_ = dynacore::Vector::Constant(humanoid::num_act_joint, -500.);
    wblc_data_->tau_max_ = dynacore::Vector::Constant(humanoid::num_act_joint, 500.);
...
void SingleContactTransCtrl::OneStep(void* _cmd){
    ...
    for(int i(0); i<humanoid::num_act_joint; ++i){
        ((Humanoid_Command*)_cmd)->jtorque_cmd[i] = gamma[i];
        ((Humanoid_Command*)_cmd)->jpos_cmd[i] = des_jpos_[i];
        ((Humanoid_Command*)_cmd)->jvel_cmd[i] = des_jvel_[i];
    }
...
void SingleContactTransCtrl::_compute_torque_wblc(dynacore::Vector & gamma){
    ...
    dynacore::Vector des_jacc_cmd = des_jacc_ 
        + Kp_.cwiseProduct(des_jpos_ - 
                sp_->Q_.segment(humanoid::num_virtual, humanoid::num_act_joint))
        + Kd_.cwiseProduct(des_jvel_ - sp_->Qdot_.tail(humanoid::num_act_joint));
...
void SingleContactTransCtrl::_task_setup(){
    ...
    dynacore::Vector jvel_des(humanoid::num_act_joint); jvel_des.setZero();
    dynacore::Vector jacc_des(humanoid::num_act_joint); jacc_des.setZero();
    ...
    if(b_increase_){
        if(moving_foot_ == humanoid_link::rightFoot){
            int swing_jidx = humanoid_joint::r_leg_hpz - humanoid::num_virtual;
            double h(state_machine_time_/end_time_);

            for(int i(0); i < humanoid::num_leg_joint; ++i){
                des_jpos_[i + swing_jidx] *= h; 
                des_jpos_[i + swing_jidx] += 
                    (1. - h)*sp_->des_jpos_prev_[swing_jidx + i];
             }
        }
        else if(moving_foot_ == humanoid_link::leftFoot){
            int swing_jidx = humanoid_joint::l_leg_hpz - humanoid::num_virtual;
            double h(state_machine_time_/end_time_);
...
void SingleContactTransCtrl::_contact_setup(){
    ...
    int jidx_offset(0);
    if(moving_foot_ == humanoid_link::leftFoot) {
    ...
    else if(moving_foot_ == humanoid_link::rightFoot) {
...
void SingleContactTransCtrl::FirstVisit(){
    ...
    robot_sys_->getPos(humanoid_link::torso, ini_body_pos_);
}
...
void SingleContactTransCtrl::CtrlInitialization(const std::string & setting_file_name){
    ...
    ParamHandler handler(HumanoidConfigPath + setting_file_name + ".yaml");
