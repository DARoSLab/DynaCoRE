...
#include <Humanoid_Controller/StateEstimator/BasicAccumulation.hpp>

Humanoid_StateEstimator::Humanoid_StateEstimator(RobotSystem* robot):
    curr_config_(humanoid::num_q),
    curr_qdot_(humanoid::num_qdot)
{
...
Humanoid_StateEstimator::~Humanoid_StateEstimator(){
    delete ori_est_;
}

void Humanoid_StateEstimator::Initialization(Humanoid_SensorData* data){
    curr_config_.setZero();
    curr_qdot_.setZero();
    curr_config_[humanoid::num_qdot] = 1.;

    // Joint Set
    for (int i(0); i<humanoid::num_act_joint; ++i){
        curr_config_[humanoid::num_virtual + i] = data->jpos[i];
        curr_qdot_[humanoid::num_virtual + i] = data->jvel[i];
    }
    ...
    curr_config_[humanoid::num_qdot] = body_ori.w();
    ...
    sp_->jpos_ini_ = 
        curr_config_.segment(humanoid::num_virtual, humanoid::num_act_joint);
...
void Humanoid_StateEstimator::Update(Humanoid_SensorData* data){
    curr_config_.setZero();
    curr_qdot_.setZero();
    curr_config_[humanoid::num_qdot] = 1.;
