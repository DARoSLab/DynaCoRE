...
CoMFootTask::CoMFootTask(RobotSystem* robot, int swing_foot):WBDC_Relax_Task(9),
    robot_sys_(robot),
                                           swing_foot_(swing_foot)
{
  ...
  // printf("[CoMFoot Task] Constructed\n");
}

CoMFootTask::~CoMFootTask(){}

bool CoMFootTask::_UpdateCommand(void* pos_des,
...
bool CoMFootTask::_UpdateTaskJacobian(){
  dynacore::Matrix Jcom, Jfoot;
  ...
  // dynacore::pretty_print(Jt_, std::cout, "Jt CoMFoot");
  return true;
}

bool CoMFootTask::_UpdateTaskJDotQdot(){
  // TODO
