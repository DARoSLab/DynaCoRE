...
void Mercury_InvKinematics::getFootPos(int link_id, const dynacore::Vector & guess_Q, dynacore::Vect3 & foot_pos){
    dynacore::Vector config = guess_Q;
...
void Mercury_InvKinematics::solveFullInvKinematics(
        double des_height,
    ...
    config_sol = dynacore::Vector::Zero(mercury::num_q);
    config_sol[2] = des_height;
    config_sol[3] = des_quat.x();
    config_sol[4] = des_quat.y();
    config_sol[5] = des_quat.z();
    config_sol[mercury_joint::virtual_Rw] = des_quat.w();
    config_sol[mercury_joint::rightKnee] = 0.2;
    config_sol[mercury_joint::leftKnee] = 0.2;


    // Find Foot body id
    unsigned int bodyid;
    int leg_jidx;
    int lfootID = model_->GetBodyId("lfoot");
    int rfootID = model_->GetBodyId("rfoot");
    ...
    dynacore::Vect3 pos_tmp;
    dynacore::Vect3 zero_vec; zero_vec.setZero();
    dynacore::Matrix J, Jtmp, Jinv;
    Jtmp = dynacore::Matrix::Zero(3, mercury::num_qdot);
    ...
    void Mercury_InvKinematics::getLegConfigAtVerticalPosture(
        ...
        void Mercury_InvKinematics::getSingleSupportStanceLegConfiguration(
                const dynacore::Vector & current_Q,
                const dynacore::Quaternion & des_quat,
                const double & des_height, 
                int swing_foot_,
