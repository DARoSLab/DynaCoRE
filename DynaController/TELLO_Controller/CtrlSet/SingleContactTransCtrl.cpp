...
void SingleContactTransCtrl::_task_setup(){
    ...
    //if(b_increase_){
        //if(moving_foot_ == tello_link::rightFoot){
            //int swing_jidx = tello_joint::r_hip_yaw - tello::num_virtual;
            //double h(state_machine_time_/end_time_);

            //for(int i(0); i < tello::num_leg_joint; ++i){
                //des_jpos_[i + swing_jidx] *= h; 
                //des_jpos_[i + swing_jidx] += 
                    //(1. - h)*sp_->des_jpos_prev_[swing_jidx + i];
             //}
        //}
        //else if(moving_foot_ == tello_link::leftFoot){
            //int swing_jidx = tello_joint::l_hip_yaw - tello::num_virtual;
            //double h(state_machine_time_/end_time_);
    ...
    //dynacore::pretty_print(sp_->des_jpos_prev_, std::cout, "jpos pre");
...
void SingleContactTransCtrl::_contact_setup(){
    ...
    int jidx_offset(0);
    if(moving_foot_ == tello_link::leftFoot) {
        ...
        ((PitchContact*)lfoot_contact_)->setMaxFz(upper_lim); 
    }
    else if(moving_foot_ == tello_link::rightFoot) {
        ...
        ((PitchContact*)rfoot_contact_)->setMaxFz(upper_lim); 
    }
}
