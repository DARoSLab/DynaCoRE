...
#include <TELLO_Controller/ContactSet/PitchContact.hpp>
#include <TELLO_Controller/ContactSet/PitchYawContact.hpp>
#include <TELLO_Controller/ContactSet/PointContact.hpp>
...
{
    des_jacc_.setZero();
    //rfoot_contact_ = new PointContact(robot_sys_, tello_link::rightFoot);
    //lfoot_contact_ = new PointContact(robot_sys_, tello_link::leftFoot);

    //rfoot_contact_ = new PitchContact(robot_sys_, tello_link::rightFoot);
    //lfoot_contact_ = new PitchContact(robot_sys_, tello_link::leftFoot);
    
    rfoot_contact_ = new PitchYawContact(robot_sys_, tello_link::rightFoot);
    lfoot_contact_ = new PitchYawContact(robot_sys_, tello_link::leftFoot);
     dim_contact_ = rfoot_contact_->getDim() + lfoot_contact_->getDim();

    body_pos_task_ = new LinkPosTask(robot_sys_, tello_link::torso);
    body_ori_task_ = new LinkOriTask(robot_sys_, tello_link::torso);
    ...
    //wblc_data_->W_qddot_ = dynacore::Vector::Constant(tello::num_qdot, 10.0);
    ...
    wblc_data_->tau_min_ = dynacore::Vector::Constant(tello::num_act_joint, -500.);
    wblc_data_->tau_max_ = dynacore::Vector::Constant(tello::num_act_joint, 500.);

    kin_wbc_contact_list_.clear();
    int rf_idx_offset(0);
    if(swing_foot == tello_link::leftFoot) {
        ...
        //wblc_data_->W_xddot_[0 + rf_idx_offset] = 1000.001;//Pitch restriction
        wblc_data_->W_rf_[lfoot_contact_->getFzIndex() + rf_idx_offset] = 0.5;

        ((PitchYawContact*)lfoot_contact_)->setMaxFz(0.0001); 
        kin_wbc_contact_list_.push_back(rfoot_contact_);
    }
    else if(swing_foot == tello_link::rightFoot) {
        ...
        ((PitchYawContact*)rfoot_contact_)->setMaxFz(0.0001); 
...
void BodyFootPlanningCtrl::OneStep(void* _cmd){   
    ...
    TELLO_Command* cmd = static_cast<TELLO_Command*> (_cmd);
    ...
    for(int i(0); i<tello::num_act_joint; ++i){
        cmd->jtorque_cmd[i] = gamma[i];
        cmd->jpos_cmd[i] = des_jpos_[i];
        cmd->jvel_cmd[i] = des_jvel_[i];
    }
    // Ankle
    //cmd->jtorque_cmd[tello_joint::r_ankle - tello::num_virtual] = 0.;
    //cmd->jtorque_cmd[tello_joint::l_ankle - tello::num_virtual] = 0.;

    //cmd->jvel_cmd[tello_joint::r_ankle - tello::num_virtual] = 0.;
    //cmd->jvel_cmd[tello_joint::l_ankle - tello::num_virtual] = 0.;

    //cmd->jpos_cmd[tello_joint::r_ankle - tello::num_virtual] = sp_->Q_[tello_joint::r_ankle];
    //cmd->jpos_cmd[tello_joint::l_ankle - tello::num_virtual] = sp_->Q_[tello_joint::l_ankle];
...
void BodyFootPlanningCtrl::_compute_torque_wblc(dynacore::Vector & gamma){
    ...
    //dynacore::pretty_print(des_jacc_cmd, std::cout, "des_jacc_cmd");
...
void BodyFootPlanningCtrl::_task_setup(){
    ...
    dynacore::Vector zero(tello::num_act_joint); zero.setZero();
...
void BodyFootPlanningCtrl::_Replanning(dynacore::Vect3 & target_loc){
    ...
    if(swing_foot_ == tello_link::leftFoot)
...
bool BodyFootPlanningCtrl::EndOfPhase(){
    ...
    if(b_contact_switch_check_){
        ...
        if(swing_foot_ == tello_link::leftFoot && sp_->b_lfoot_contact_){
            contact_happen = true;
        }
        if(swing_foot_ == tello_link::rightFoot && sp_->b_rfoot_contact_){
            contact_happen = true;
        }
...
void BodyFootPlanningCtrl::CtrlInitialization(
    ...
    ini_base_height_ = sp_->Q_[tello_joint::virtual_Z];
