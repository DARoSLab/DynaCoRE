...
#include <TELLO_Controller/TaskSet/SelectedJPosTask.hpp>
...
void BodyCtrl::OneStep(void* _cmd){
    ...
    dynacore::Vector gamma = dynacore::Vector::Zero(tello::num_act_joint);
    _contact_setup();
    _task_setup();
...
void BodyCtrl::_task_setup(){
    ...
    //des_pos[2] = body_height_cmd + (-0.1 + 0.1*cos(state_machine_time_*M_PI) );
...
void BodyCtrl::FirstVisit(){
    jpos_ini_ = sp_->Q_.segment(tello::num_virtual, tello::num_act_joint);
