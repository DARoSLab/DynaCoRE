...
#include <TELLO_Controller/CtrlSet/DoubleContactTransCtrl.hpp>
#include <TELLO_Controller/CtrlSet/BodyCtrl.hpp>
#include <TELLO_Controller/CtrlSet/SingleContactTransCtrl.hpp>
#include <TELLO_Controller/CtrlSet/BodyFootPlanningCtrl.hpp>
    ...
    num_step_(0)
{
    ...
    sp_->stance_foot_ = tello_link::leftFoot;
    ...
    right_swing_start_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, tello_link::rightFoot, false);
    right_swing_end_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, tello_link::rightFoot, true);
    // Left
    left_swing_start_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, tello_link::leftFoot, false);
    left_swing_end_trans_ctrl_ = 
        new SingleContactTransCtrl(robot, tello_link::leftFoot, true);
    // Swing Controller Selection
    right_swing_ctrl_ = 
        new BodyFootPlanningCtrl(robot_sys_, tello_link::rightFoot, reversal_planner_);
    left_swing_ctrl_ = 
        new BodyFootPlanningCtrl(robot_sys_, tello_link::leftFoot, reversal_planner_);
...
void WalkingTest::TestInitialization(){
    ...
    reversal_planner_->PlannerInitialization(
            TELLOConfigPath"PLANNER_velocity_reversal");
    // Yaml file name
...
int WalkingTest::_NextPhase(const int & phase){
    ...
    if(phase == WkPhase::double_contact_1) {
        ...
        robot_sys_->getPos(tello_link::leftFoot, next_local_frame_location);
    ...
    if(phase == WkPhase::double_contact_2){
        ++num_step_;
        sp_->stance_foot_ = tello_link::rightFoot;

        // Global Frame Update
        dynacore::Vect3 next_local_frame_location;
        robot_sys_->getPos(tello_link::rightFoot, next_local_frame_location);
        sp_->global_pos_local_ += next_local_frame_location;
...
void WalkingTest::_SettingParameter(){
    // Setting Parameters
    ParamHandler handler(TELLOConfigPath"TEST_walking.yaml");
