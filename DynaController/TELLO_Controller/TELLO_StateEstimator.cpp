...
#include <TELLO_Controller/StateEstimator/BasicAccumulation.hpp>

TELLO_StateEstimator::TELLO_StateEstimator(RobotSystem* robot):
    curr_config_(tello::num_q),
    curr_qdot_(tello::num_qdot)
{
...
TELLO_StateEstimator::~TELLO_StateEstimator(){
    delete ori_est_;
}

void TELLO_StateEstimator::Initialization(TELLO_SensorData* data){
    curr_config_.setZero();
    curr_qdot_.setZero();
    curr_config_[tello::num_qdot] = 1.;

    // Joint Set
    for (int i(0); i<tello::num_act_joint; ++i){
        curr_config_[tello::num_virtual + i] = data->jpos[i];
        curr_qdot_[tello::num_virtual + i] = data->jvel[i];
    }
    ...
    curr_config_[tello::num_qdot] = body_ori.w();
    ...
    sp_->jpos_ini_ = 
        curr_config_.segment(tello::num_virtual, tello::num_act_joint);
...
void TELLO_StateEstimator::Update(TELLO_SensorData* data){
    curr_config_.setZero();
    curr_qdot_.setZero();
    curr_config_[tello::num_qdot] = 1.;
