...
Humanoid_Kin_Model::Humanoid_Kin_Model( RigidBodyDynamics::Model* model){
    model_ = model;
    Ig_ = dynacore::Matrix::Zero(6,6);
    Jg_ = dynacore::Matrix::Zero(6, model_->qdot_size);
}

Humanoid_Kin_Model::~Humanoid_Kin_Model(){
}

void Humanoid_Kin_Model::UpdateKinematics(const dynacore::Vector & q, const dynacore::Vector & qdot){
    _UpdateCentroidFrame(q, qdot);
}

void Humanoid_Kin_Model::_UpdateCentroidFrame_diff(const dynacore::Vector & q, const dynacore::Vector & qdot){
    double mass;
    ...
    int start_idx = _find_body_idx(humanoid_link::torso);
    Matrix3d p;
    Matrix3d cmm;
    Matrix3d R;
...
void Humanoid_Kin_Model::_UpdateCentroidFrame(const dynacore::Vector & q, 
...
void Humanoid_Kin_Model::getCoMJacobian(dynacore::Matrix & Jcom) {
...
void Humanoid_Kin_Model::getCoMPos(dynacore::Vect3 & CoM_pos) {
    Vector3d zero_vector = Vector3d::Zero();
    dynacore::Vector q;
...
void Humanoid_Kin_Model::getCoMVel(dynacore::Vect3 & CoM_vel) {
...
void Humanoid_Kin_Model::getPos(int link_id, dynacore::Vect3 & pos){
    Vector3d zero;
    dynacore::Vector q;
...
void Humanoid_Kin_Model::getOri(int link_id, dynacore::Quaternion & ori){
    Matrix3d R;
    dynacore::Vector q;
...
void Humanoid_Kin_Model::getLinearVel(int link_id, dynacore::Vect3 & vel){
    Vector3d zero;
    dynacore::Vector q, qdot;
...
void Humanoid_Kin_Model::getAngularVel(int link_id, dynacore::Vect3 & ang_vel){
...
void Humanoid_Kin_Model::getJacobian(int link_id, dynacore::Matrix &J){
    dynacore::Vector q;
...
void Humanoid_Kin_Model::getJDotQdot(int link_id, dynacore::Vector & JDotQdot){
...
unsigned int Humanoid_Kin_Model::_find_body_idx(int id) const {
    switch(id){
        case humanoid_link::torso:
            return model_->GetBodyId("torso");
        case humanoid_link::rightFoot:
            return model_->GetBodyId("R_Foot");
        case humanoid_link::leftFoot:
