...
Humanoid_Dyn_Model::Humanoid_Dyn_Model(RigidBodyDynamics::Model* model){
    model_ = model;
}

Humanoid_Dyn_Model::~Humanoid_Dyn_Model(){
}

bool Humanoid_Dyn_Model::getMassInertia(dynacore::Matrix & a){
    a = A_;
    return true;
}

bool Humanoid_Dyn_Model::getInverseMassInertia(dynacore::Matrix & ainv){
    ainv = Ainv_;
    return true;
}

bool Humanoid_Dyn_Model::getGravity(dynacore::Vector &  grav){
    grav = grav_;
    return true;
}
bool Humanoid_Dyn_Model::getCoriolis(dynacore::Vector & coriolis){
    coriolis = coriolis_;
    return true;
}

void Humanoid_Dyn_Model::UpdateDynamics(const dynacore::Vector & q, const dynacore::Vector & qdot){
    // Mass Matrix
