...
DracoModel::DracoModel(){
  ...
  Body vlink_x, link_body, link_thigh, link_shank, link_foot, link_toe, link_heel;
  Joint vjoint_x, vjoint_z, joint_ry;
  ...
  ///                 Assemble Model                 ///
  //////////////////////////////////////////////////////
  Vector3d com_pos, gyration_radii;
  com_pos.setZero();
  gyration_radii.setZero();
  double mass_zero(1.e-8);
  joint_ry = Joint (JointTypeRevolute, Vector3d (0., 1., 0.) );

  // ground to Virtual X link
  vlink_x = Body (mass_zero, com_pos, gyration_radii);
  vjoint_x = Joint(JointTypePrismatic, Vector3d (1., 0., 0.)  );
  int vlx_id = model_->AddBody(0, Xtrans(Vector3d(0., 0., 0.)), vjoint_x, vlink_x, "virtual_x");
  printf("body X virtual id: %i \n", vlx_id);

  // Omitting rotation //
  // Virtual X link to Body
  link_body = Body (mass_body, body_com, inertia_body);
  vjoint_z = Joint(JointTypePrismatic, Vector3d (0., 0., 1.)  );
  int body_id = model_->AddBody(vlx_id, Xtrans(Vector3d(0.,0.,0.)), vjoint_z, link_body, "body");
  printf("Body id: %i \n", body_id);
  ...
  com_pos.setZero();
  gyration_radii.setZero();
  link_toe = Body(mass_zero, com_pos, gyration_radii);
  int toe_id = model_->AddBody(foot_id, Xtrans(Vector3d(lx_toe, 0., lz_foot)), fixed_joint, link_toe, "toe");
  printf("toe id: %i \n", toe_id);

  // Fixed Joint (Heel)
  link_heel = Body(mass_zero, com_pos, gyration_radii);
