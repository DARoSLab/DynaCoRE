...
Walker2D_Dyn_Model::Walker2D_Dyn_Model(RigidBodyDynamics::Model* model){
    model_ = model;
}

Walker2D_Dyn_Model::~Walker2D_Dyn_Model(){
}

bool Walker2D_Dyn_Model::getMassInertia(Matrix & a){
    a = A_;
    return true;
}

bool Walker2D_Dyn_Model::getInverseMassInertia(Matrix & ainv){
    ainv = Ainv_;
    return true;
}

bool Walker2D_Dyn_Model::getGravity(Vector &  grav){
    grav = grav_;
    return true;
}
bool Walker2D_Dyn_Model::getCoriolis(Vector & coriolis){
    coriolis = coriolis_;
    return true;
}

void Walker2D_Dyn_Model::UpdateDynamics(const sejong::Vector & q, const sejong::Vector & qdot){
    // Mass Matrix
