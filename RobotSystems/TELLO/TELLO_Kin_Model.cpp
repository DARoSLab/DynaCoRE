...
TELLO_Kin_Model::TELLO_Kin_Model( RigidBodyDynamics::Model* model){
    model_ = model;
    Ig_ = dynacore::Matrix::Zero(6,6);
    Jg_ = dynacore::Matrix::Zero(6, model_->qdot_size);
}

TELLO_Kin_Model::~TELLO_Kin_Model(){
}

void TELLO_Kin_Model::UpdateKinematics(const dynacore::Vector & q, const dynacore::Vector & qdot){
    _UpdateCentroidFrame(q, qdot);
}

void TELLO_Kin_Model::_UpdateCentroidFrame_diff(const dynacore::Vector & q, const dynacore::Vector & qdot){
    double mass;
    ...
    int start_idx = _find_body_idx(tello_link::torso);
    Matrix3d p;
    Matrix3d cmm;
    Matrix3d R;
...
void TELLO_Kin_Model::_UpdateCentroidFrame(const dynacore::Vector & q, 
...
void TELLO_Kin_Model::getCoMJacobian(dynacore::Matrix & Jcom) {
...
void TELLO_Kin_Model::getCoMPos(dynacore::Vect3 & CoM_pos) {
    Vector3d zero_vector = Vector3d::Zero();
    dynacore::Vector q;
...
void TELLO_Kin_Model::getCoMVel(dynacore::Vect3 & CoM_vel) {
...
void TELLO_Kin_Model::getPos(int link_id, dynacore::Vect3 & pos){
    Vector3d zero;
    dynacore::Vector q;
...
void TELLO_Kin_Model::getOri(int link_id, dynacore::Quaternion & ori){
    Matrix3d R;
    dynacore::Vector q;
...
void TELLO_Kin_Model::getLinearVel(int link_id, dynacore::Vect3 & vel){
    Vector3d zero;
    dynacore::Vector q, qdot;
...
void TELLO_Kin_Model::getAngularVel(int link_id, dynacore::Vect3 & ang_vel){
...
void TELLO_Kin_Model::getJacobian(int link_id, dynacore::Matrix &J){
    dynacore::Vector q;
...
void TELLO_Kin_Model::getJDotQdot(int link_id, dynacore::Vector & JDotQdot){
...
unsigned int TELLO_Kin_Model::_find_body_idx(int id) const {
    switch(id){
        case tello_link::torso:
            return model_->GetBodyId("BODY");
        case tello_link::rightFoot:
            return model_->GetBodyId("R_FOOT");
        case tello_link::leftFoot:
