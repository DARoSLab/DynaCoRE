...
TELLO_Dyn_Model::TELLO_Dyn_Model(RigidBodyDynamics::Model* model){
    model_ = model;
}

TELLO_Dyn_Model::~TELLO_Dyn_Model(){
}

bool TELLO_Dyn_Model::getMassInertia(dynacore::Matrix & a){
    a = A_;
    return true;
}

bool TELLO_Dyn_Model::getInverseMassInertia(dynacore::Matrix & ainv){
    ainv = Ainv_;
    return true;
}

bool TELLO_Dyn_Model::getGravity(dynacore::Vector &  grav){
    grav = grav_;
    return true;
}
bool TELLO_Dyn_Model::getCoriolis(dynacore::Vector & coriolis){
    coriolis = coriolis_;
    return true;
}

void TELLO_Dyn_Model::UpdateDynamics(const dynacore::Vector & q, const dynacore::Vector & qdot){
    // Mass Matrix
