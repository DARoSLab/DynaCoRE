...
OC3_Kin_Model::OC3_Kin_Model( RigidBodyDynamics::Model* model){
  model_ = model;
  Ig_ = Matrix::Zero(6,6);
  Jg_ = Matrix::Zero(6, model_->qdot_size);
}

OC3_Kin_Model::~OC3_Kin_Model(){
}
void OC3_Kin_Model::UpdateKinematics(const sejong::Vector & q, const sejong::Vector & qdot){
  _UpdateCentroidFrame(q, qdot);
}

void OC3_Kin_Model::_UpdateCentroidFrame(const sejong::Vector & q, const sejong::Vector & qdot){
  double mass;
  ...
  int start_idx = _find_body_idx(LK_L1);
  Matrix3d p;
  Matrix3d cmm;
  Matrix3d R;
...
void OC3_Kin_Model::getCoMJacobian(const sejong::Vector & Q, sejong::Matrix & Jcom) const {
...
void OC3_Kin_Model::getCoMPos(const sejong::Vector & q, sejong::Vect3 & CoM_pos, bool update)const {
  Vector3d zero_vector = Vector3d::Zero();
...
void OC3_Kin_Model::getCoMVel(const sejong::Vector & q, const sejong::Vector & qdot, sejong::Vect3 & CoM_vel) const {
...
void OC3_Kin_Model::getPosition(const Vector & q, int link_id, Vect3 & pos){
  Vector3d zero;
  // zero << 0.0, 0.0, 0.0;
...
void OC3_Kin_Model::getOrientation(const Vector & q, int link_id, sejong::Quaternion & ori){
  Matrix3d R;
...
void OC3_Kin_Model::getVelocity(const Vector & q, const Vector &qdot,
...
void OC3_Kin_Model::getAngVel(const Vector & q, const Vector & qdot,
...
void OC3_Kin_Model::getJacobian(const Vector & q, int link_id, Matrix &J){
...
void OC3_Kin_Model::getJacobianDot6D_Analytic(const Vector & q, const Vector & qdot, int link_id, Matrix & J){
  J = Matrix::Zero(6, model_->qdot_size);
...
unsigned int OC3_Kin_Model::_find_body_idx(int id) const {
  switch(id){
  case LK_L1:
