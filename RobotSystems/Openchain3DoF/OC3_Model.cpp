#include "OC3_Model.hpp"
#include "OC3_Dyn_Model.hpp"
#include "OC3_Kin_Model.hpp"
...
OC3Model* OC3Model::GetOC3Model(){
    static OC3Model draco_model_;
    return & draco_model_;
}
...
OC3Model::OC3Model(){
  ...
  Body link1, link2, link3, link_ee;
  ...
  //////////////////////////////////////////////////////
    ...
    dyn_model_ = new OC3_Dyn_Model(model_);
    kin_model_ = new OC3_Kin_Model(model_);
...
void OC3Model::UpdateModel(const Vector & q, const Vector & qdot){
    UpdateKinematicsCustom(*model_, &q, &qdot, NULL);
    dyn_model_->UpdateDynamics(q, qdot);
    kin_model_->UpdateKinematics(q, qdot);
}

void OC3Model::getCentroidInertia(sejong::Matrix & Icent){
...
void OC3Model::getCentroidJacobian(sejong::Matrix & Jcent){
...
void OC3Model::UpdateKinematics(const Vector & q, const Vector &qdot){
    UpdateKinematicsCustom(*model_, &q, &qdot, NULL);
}

bool OC3Model::getInverseMassInertia(sejong::Matrix & Ainv) {
    return dyn_model_->getInverseMassInertia(Ainv);
}

bool OC3Model::getMassInertia(sejong::Matrix & A) {
    return dyn_model_->getMassInertia(A);
}

bool OC3Model::getGravity(Vector & grav) {
    return dyn_model_->getGravity(grav);
}

bool OC3Model::getCoriolis(Vector & coriolis) {
    return dyn_model_->getCoriolis(coriolis);
}

void OC3Model::getFullJacobian(const Vector & q, int link_id, sejong::Matrix & J) const {
...
void OC3Model::getFullJacobianDot(const Vector & q, const Vector & qdot, int link_id, sejong::Matrix & Jdot) const {
...
void OC3Model::getPosition(const Vector & q,
...
void OC3Model::getOrientation(const Vector & q,
                               int link_id, sejong::Quaternion & ori) {
  kin_model_->getOrientation(q, link_id, ori);
}

void OC3Model::getVelocity(const Vector & q, const Vector &qdot,
...
void OC3Model::getAngVel(const Vector & q, const Vector & qdot,
                          int link_id, Vect3 & ang_vel){
    kin_model_->getAngVel(q, qdot, link_id, ang_vel);
}

void OC3Model::getCoMJacobian(const Vector & q, sejong::Matrix & J){
    J = sejong::Matrix::Zero(3, model_->qdot_size);
    kin_model_->getCoMJacobian(q, J);
}

void OC3Model::getCoMPosition(const Vector & q, Vect3 & com_pos, bool update){
  com_pos = kin_model_->com_pos_;
    // kin_model_->getCoMPos(q, com_pos, update);
}

void OC3Model::getCoMVelocity(const Vector & q, const Vector & qdot, Vect3 & com_vel){
    kin_model_->getCoMVel(q, qdot, com_vel);
}
void OC3Model::getCentroidVelocity(sejong::Vector & centroid_vel){
